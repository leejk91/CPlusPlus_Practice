...
static unsigned char HW_Version[16]    = "V1.0";
static unsigned char FW_Version[16]    = "Maru1.0";		// GATE_OLD_VERSION
static unsigned char FW_DateTime[16]   = "20:04:17:11";	// YY:MM:DD:hh
...
void VM_GetConfigRead(void)
{
    ...
    fp = fopen("/app/conf/Config.dat", "r");
	if(fp!=NULL) {
...
int VM_GetConfigVersion(unsigned char *pVer)
{
    ...
    strcpy((char *)pVer, (const char *)FW_Version);
	
    if( memcmp(&g_VmConfig.Version[0], FW_Version, strlen((const char*)FW_Version)) || memcmp(&g_VmConfig.Version[1], HW_Version, strlen((const char*)HW_Version)) ) {
		
		strcpy((char *)&g_VmConfig.Version[0],  (char *)FW_Version);
		strcpy((char *)&g_VmConfig.Version[1],  (char *)HW_Version);
		
        fp = fopen("/app/conf/Config.dat", "w");
		if(fp!=NULL) {
    ...
    strcpy(devinfo.date_time, (char *)FW_DateTime);
	
    strcpy((char *)pVer+5, (char *)FW_DateTime);
...
void VM_SetConfigTID(unsigned char *pTID, int nLength)
{
    ...
    memcpy(g_VmConfig.TID, pTID, (size_t)nLength);
	g_VmConfig.TID[nLength] = 0;

    fp = fopen("/app/conf/Config.dat", "wb");
	if(fp!=NULL) {
		fwrite(&g_VmConfig, sizeof(VM_CONFIG), 1, fp);	
		fclose(fp);
	}
...
void VM_SetConfigTID1(unsigned char *pTID, int nLength)
{
    ...
    memcpy(g_VmConfig.TID1, pTID, (size_t)nLength);
...
void VM_SetConfigTID2(unsigned char *pTID, int nLength)
{
    ...
    memcpy(g_VmConfig.TID2, pTID, (size_t)nLength);
...
int VM_GetConfigTID(unsigned char *pTID)
{
    ...
    fp = fopen("/app/conf/Config.dat", "rb");
	if(fp!=NULL) {
		fread(&g_VmConfig, sizeof(VM_CONFIG), 1, fp);	
		fclose(fp);

        strcpy((char *)pTID, (char*)g_VmConfig.TID);
        return (int)strlen((char *)g_VmConfig.TID);
	}
	pTID[0]=0;
	return 0;
}
...
int VM_GetConfigTID1(unsigned char *pTID)
{
	...
	if(fp!=NULL) {
        ...
        strcpy((char *)pTID, (char *)g_VmConfig.TID1);
        return (int)strlen((char *)g_VmConfig.TID1);
	}
	pTID[0]=0;
	return 0;
}
...
int VM_GetConfigTID2(unsigned char *pTID)
{
	...
	if(fp!=NULL) {
        ...
        strcpy((char *)pTID, (char *)g_VmConfig.TID2);
        return (int)strlen((char*)g_VmConfig.TID2);
	}
	pTID[0]=0;
	return 0;
}
...
void VM_SetConfigCatID(unsigned char *pTID, int nLength)
{
    ...
    memcpy(g_VmConfig.CATID, pTID, (size_t)nLength);
...
int VM_GetConfigCatID(unsigned char *pTID)
{
	...
	if(fp!=NULL) {
        ...
        strcpy((char *)pTID, (char *)g_VmConfig.CATID);
        return (int)strlen((char*)g_VmConfig.CATID);
	}
	pTID[0]=0;
	return 0;
}
...
void save_self_verification(int result)
{
    FILE *fp;
    int i = 0;
    unsigned char g_Today[7];

    for(i=4;i>0;i--)
    {
        g_VmConfig.comp_result[i] = g_VmConfig.comp_result[i-1];
        memcpy(g_VmConfig.comp_date[i],g_VmConfig.comp_date[i-1],5);
    }

    g_VmConfig.comp_result[0] = (char)(result+0x08);

    GetTimeBCD((time_t)NULL, (char *)g_Today);
    BCD2Char(&g_Today[2],(unsigned char*)g_VmConfig.comp_date[0],2);
