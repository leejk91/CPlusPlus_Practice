...
NanoVANBasicReq g_NanoVANBasicReq, g_CurrVANBasicReq;
...
int g_CreditAuth=0, g_CraditType=0, g_CraditTimeout=0, g_CraditComplete=0, g_CraditCancel=0, g_CraditPrevCancel=0;
...
int UBC_ReadVanFlash(unsigned char *rData)
{
    int  nLen = 0;
    unsigned short crc;

    nLen = Char2Int(g_CurrVANBasicReq.MsgLen, 0) + 5;
    crc = (unsigned short)CRC_Update((unsigned char *)&g_CurrVANBasicReq, (unsigned short)(nLen+2));

    if(crc) {
        nLen=0;
        PRINTF( "\r\nVAN CRC VERIFY %04X\r\n", crc);
        HEXDUMP((unsigned char*)&g_CurrVANBasicReq, nLen,  "ReadVan");
        return 0;
    }

    if(nLen != 5) {
        memcpy(rData, (unsigned char *)&g_CurrVANBasicReq, (size_t)nLen);
        HEXDUMP( rData, nLen, "\r\nrData > %d", nLen);
    }
    else {
        nLen = 0;
...
void UBC_NanoPop(void) 
{
	...
	{	
		...
		if(g_LogPosition==1) 
		{	// 휘발상 파일 삭제	
            sprintf(rm_buf, "rm /app/tmp/transaction_%d.dat\0 &", g_NanoBackup.pop);
			system(rm_buf);
		}
		else {
			// 비휘발성 신용망상 승인, 선불/ 현금 수집 삭제
            sprintf(rm_buf, "rm /app/log/transaction_%d.dat\0 &", g_NanoBackup.pop);
			system(rm_buf);
		}
        ...
        fp = fopen("/app/log/trans_counter.dat", "w");
		if(fp!=NULL) {
...
int UBC_ChkWarningCode(void)
{
    unsigned short sCode;
    unsigned char tBuf[8];

    Char2BCD(VAN_BasicRes.ResCode, tBuf, 4);
    sCode = B2S(tBuf[1], tBuf[0]);
    ...
    case 0x7004 :	// 전문오류,  ........
        return -1;
    case 0x5110 :
    case 0x5113 :	// 잠시후 재 요망
    case 0x7002 :	// 이미 취소 된 거래
    case 0x7003 :	// 선불 수집 취소
    case 0x7007 :	// 페이코
    case 0x7111 : 	// 카드번호 오류
    case 0x7140 :	// 거래내역 없슴
    case 0x7118 :	// 카드사 통신장애
    case 0x7119 :	// 승인 거래 불가
    case 0x7179 : 	//
    case 0x7181 :	// 수기입력번호오류
    case 0x7121 :	// 페이코
    case 0x8032 :	// 거래금액너무적음
    case 0x8314 :	// 유효기간 경과 카드
    case 0x8324 :	// 거래정지 카드
    case 0x8350 :	// 도난, 분실
    case 0x8373 :	// 카드사 전화 요망
    case 0x8411 :	// 이미 취소 된 거래
        return -2;
    default :
        if(!memcmp(VAN_BasicRes.ResCode, "-", 1)) return -3;	// 전문 오류
    }
...
{
    int i, chk, nRes=RC_SUCCESS, nCreditOk=RC_FAILURE;
    ...
    // 28.	6D an	15	가맹점 번호
    if(*pData != 0x1C) {
        memset(VAN_BasicRes.MerchantID, 0x20, sizeof(VAN_BasicRes.MerchantID));
        for(i=0; pData[i]!=0x1C && i<15; i++) VAN_BasicRes.MerchantID[i]=pData[i];
    ...
    // 32.	9D an	20	발급사 명
    if(*pData != 0x1C) {
        for(i=0; pData[i]!=0x1C && i<20; i++) VAN_BasicRes.IssuerName[i]=pData[i];
    ...
    // 34.	B2 an	20	매입사 명
    if(*pData != 0x1C) {
        for(i=0; pData[i]!=0x1C && i<20; i++) VAN_BasicRes.AcquirerName[i]=pData[i];
...
#
            if( !memcmp(VAN_BasicRes.TransactionClass, "0909", 4)  ) {
                PRINTF( "단말기마감정산요청\r\n");
#if 0
                adjustment_check = 2;
                GetTimeBCD((time_t)NULL, (char *)adjustment_time);

                if(checkSaleInfoMachine()) {
                    play_wav("sound/wait.wav");
                }
                else
                if(checkMdbMode()) {
                    play_wav("sound/wait.wav");
                    for(i=0; i<sale_info_push; i++) UBC_SaleInfoReq((PVM_DETAIL_INFO)sale_info_init[i]);
                }
                else {
                    // 데이터 정보가 맞지 않아서 ...
                    if(AmountInfo.PulseMode == CoCaGateCP24)	for(i=0; i<25; i++) UBC_SaleInfoReq((PVM_DETAIL_INFO)sale_info_init[i]);
                    play_wav("sound/transaction_complete.wav");
                }
#endif
            }
            else
            if( !memcmp(VAN_BasicRes.TransactionClass, "0905", 4)  )
            {
                PRINTF( "자판기판매정보집계\r\n");
#if 0
                if( checkSaleInfoMachine() ) {
                    if (vm_detail_info_push == vm_detail_info_pop) {
                        VM_MachineInfoDisplay(1);
                        UBC_SetStatusInfo(1);
                        play_wav("sound/transaction_complete.wav");
                    }
                }
                ...
                if( checkMdbMode() ) {
                    sale_info_pop = (sale_info_pop+1)%1024;


                    if (sale_info_push == sale_info_pop) {
                        ...
                        LCD_Display(LCD_SORT_CENTER, 4, 0, "거래정보전송");
                        LCD_Display(LCD_SORT_CENTER, 6, 1, "[%d/%d]", sale_info_pop, sale_info_push);
                    }
                }
#endif
            }
    ...
    Char2BCD(VAN_BasicRes.TransactionClass, tBuf, 4);
    sCode = B2S(tBuf[1], tBuf[0]);

    // 부가 정보 없을 경우  .. Exception
    // 코레일 관광, 페이코
    switch(sCode) {
    case 0x0701 : case 0x0702 : case 0x0706 : case 0x0708 :
        if(i==0) i=1;
        break;
    }
    ...
    if(i>0 && ( !memcmp(VAN_BasicRes.ResCode, "8412", 4) || !memcmp(VAN_BasicRes.ResCode, "0000", 4)) )
    {
        ...
        // HEXDUMP(VAN_BasicRes.AddedInfo, i, "AddedInfo %04X", sCode);

        switch(sCode) {
#if 0
        case 0x0701 :
            UBC_ClrPaycoTrack2();
            UBC_ClrKorailTravalQrcode();
            break;
#endif
        case 0x0101 :
        case 0x0102 :
...
//		case 0x0108 :
    ...
    if(nRes == RC_SUCCESS) {
        ...
        case 0x0106 :
        case 0x0701 :
            if( nCreditOk==RC_SUCCESS ) g_CraditComplete = 1;
            break;
        case 0x0102 :
        case 0x0108 :
        case 0x0708 :
            g_CraditPrevCancel=1;
            break;
        }
#if 0
        UBC_SetReceipt();
...
unsigned short UBC_ReadTempVANBasicReq(int nNum) 
{
    ...
    sprintf((char *)buf, "/app/log/transaction_%d.dat", nNum);
    if ((fp = fopen((char *)buf, "r")) == NULL)
        return 0;
...
{	
    ...
    sprintf(buf, "/app/log/trans_backup.dat", g_NanoBackup.push);
...
b01 :				
        sprintf((char*)buf, "/app/log/transaction_%d.dat", g_NanoBackup.push);
		...
		if(memcmp((char *)&g_NanoVANBasicReq, (char *)&VANBasicReq, sizeof(NanoVANBasicReq))) {
            //printf( "\r\n\t%1.WRITE Retry <%d>\r\n", nRetry);
            sprintf((char*)rm_buf, "rm /app/log/transaction_%d.dat\0", g_NanoBackup.push);
            system((char*)rm_buf);
...
b02 :
        ...
        sprintf((char*)buf, "/usr/rfpinpad/log/trans_counter.dat");
...
b01 :			
        sprintf((char*)buf, "/app/tmp/transaction_%d.dat", g_NanoBackup.push);
		...
		if(memcmp((char *)&g_NanoVANBasicReq, (char *)&VANBasicReq, sizeof(NanoVANBasicReq))) {
            //printf( "\r\n\t%11.WRITE Retry <%d>\r\n", nRetry);
            sprintf((char*)rm_buf, "rm /app/tmp/transaction_%d.dat\0", g_NanoBackup.push);
            system((char*)rm_buf);
			if(++nRetry<3) goto b01;
		}
