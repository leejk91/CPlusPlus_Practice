...
static void readMulti(unsigned char ucAddr, unsigned char *pBuf, int iCount)
{
	...
	if (rc == 1)
	{
        rc = read(file_i2c, pBuf, (size_t)iCount);
		if (rc != iCount) {};
	}
} /* readMulti() */
...
static void writeMulti(unsigned char ucAddr, unsigned char *pBuf, int iCount)
{
	...
	ucTemp[0] = ucAddr;
    memcpy(&ucTemp[1], pBuf, (size_t)iCount);
    rc = write(file_i2c, ucTemp, (size_t)(iCount+1));
	if (rc != iCount+1) {};
} /* writeMulti() */
...
//
static unsigned char ucI2CMode[] = {4,0x88,0x00,0x80,0x01,0xff,0x01,0x00,0x00};
static unsigned char ucI2CMode2[] = {3,0x00,0x01,0xff,0x00,0x80,0x00};
static unsigned char ucSPAD0[] = {4,0x80,0x01,0xff,0x01,0x00,0x00,0xff,0x06};
static unsigned char ucSPAD1[] = {5,0xff,0x07,0x81,0x01,0x80,0x01,0x94,0x6b,0x83,0x00};
static unsigned char ucSPAD2[] = {4,0xff,0x01,0x00,0x01,0xff,0x00,0x80,0x00};
static unsigned char ucSPAD[] = {5,0xff,0x01,0x4f,0x00,0x4e,0x2c,0xff,0x00,0xb6,0xb4};
static unsigned char ucDefTuning[] = {
...
{
    timeouts->pre_range_vcsel_period_pclks = (uint16_t)((readReg(PRE_RANGE_CONFIG_VCSEL_PERIOD) +1) << 1);
    timeouts->msrc_dss_tcc_mclks = readReg(MSRC_CONFIG_TIMEOUT_MACROP) + 1;
    timeouts->msrc_dss_tcc_us =
            timeoutMclksToMicroseconds(timeouts->msrc_dss_tcc_mclks, (uint8_t)timeouts->pre_range_vcsel_period_pclks);

    timeouts->pre_range_mclks =
            decodeTimeout(readReg16(PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI));
    timeouts->pre_range_us =
            timeoutMclksToMicroseconds(timeouts->pre_range_mclks, (uint8_t)timeouts->pre_range_vcsel_period_pclks);

    timeouts->final_range_vcsel_period_pclks = (uint16_t)((readReg(FINAL_RANGE_CONFIG_VCSEL_PERIOD) +1) << 1);
    ...
    timeouts->final_range_us =
            timeoutMclksToMicroseconds(timeouts->final_range_mclks, (uint8_t)timeouts->final_range_vcsel_period_pclks);
...
static int setVcselPulsePeriod(vcselPeriodType type, uint8_t period_pclks)
{
    ...
    if (type == VcselPeriodPreRange)
    {
        ...
        uint16_t new_pre_range_timeout_mclks =
                (uint16_t)timeoutMicrosecondsToMclks(timeouts.pre_range_us, (uint8_t)period_pclks);
        ...
        uint16_t new_msrc_timeout_mclks =
                (uint16_t)timeoutMicrosecondsToMclks(timeouts.msrc_dss_tcc_us, period_pclks);

        writeReg(MSRC_CONFIG_TIMEOUT_MACROP,
                 (new_msrc_timeout_mclks > 256) ? 255 : (unsigned char)(new_msrc_timeout_mclks - 1));
        // set_sequence_step_timeout() end
    }
    else if (type == VcselPeriodFinalRange)
    {
        ...
        uint16_t new_final_range_timeout_mclks =
                (uint16_t)timeoutMicrosecondsToMclks(timeouts.final_range_us, period_pclks);
...
static int setMeasurementTimingBudget(uint32_t budget_us)
{
    ...
    if (enables & SEQUENCE_ENABLE_FINAL_RANGE)
    {
        ...
        final_range_timeout_mclks =
                (uint16_t)timeoutMicrosecondsToMclks(final_range_timeout_us,
...
#else
        writeReg16(FINAL_RANGE_CONFIG_MIN_COUNT_RATE_RTN_LIMIT, (0.1 * (1<<7))); // 0.1
#endif
